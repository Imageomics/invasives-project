...
def get_optimal_threshold(
    ...
    # This is to adjust for the pelvic fin
    ...
    if show_plot or plot_save_filepath:  
        fig, axs = pyplot.subplots(1, len(traits_to_detect), figsize=(20, 5))
...
#     roc_aucs = list(roc_auc_score(all_targets, all_predicted, average=None))
    ...
    for i in range(all_targets.shape[1]):
        ...
        recall = recall_score(all_testy[i], preds_threshold, average='macro')
        recall = recall_score(all_testy[i], preds_threshold)
...
parser.add_argument('--name', default='trait_id_basic', type=str, help='name of experiment or run')
...
"""
python basic_evaluate.py --model_name resnet18 --checkpoint_path x.t7 --output_path x --server pda --batch_size 256 --num_workers 8
"""

#### CHANGE MODEL AND CHECKPOINT_PATH
MODEL = args.model_name
args.name = args.name + "_" + args.model_name
...
plot_save_filepath = os.path.join(args.output_path, f'{args.name}_precision_recall_plot.pdf')
...
save_filename = f'{args.name}_eval.json'
