...
def get_dataset_and_dataloader(data_file, img_dir, transform, batch_size, num_workers):
    dataset = BasicInvasivePlantsDataset(data_file, img_dir, transform)
    dataloader = DataLoader(dataset, batch_size=batch_size, num_workers=num_workers)

    return dataset, dataloader

def get_pos_weight(train_file, traits_to_detect = ["Healthy", "Leaf Miner?", "Rust?", "Other Insect? (catipillar bites, etc.)", "Mechanical? (human, animal, herbicide, etc.)"]):
