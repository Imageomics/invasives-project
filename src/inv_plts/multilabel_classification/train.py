...
def train_epoch(model, criterion, optimizer, data_loader):
    ...
    eps = 0.000001
    msg = f"Training Loss: {train_loss / (total + eps)}" 
    # | Training AP: {average_precisions} | Training  MAP: {map}"
    print(msg)

    return train_loss / (total + eps), map

def save_checkpoint(val_loss, model, optim, epoch, index=False, checkpoint_stats=None):
    # Save checkpoint.
    print('Saving..')
...
def save_checkpoint_stats_json(checkpoint_stats, epoch, index=True, type='test'):
    checkpoint_stats['best_epoch'] = epoch
    if index:
...
for epoch in range(START_EPOCH, EPOCH):
    ...
    if val_loss <= BEST_VAL:
        ...
        checkpoint_stats_k =  ['loss']
        #, 'aps', 'map', 'f1_score', 'precision', 'recall', 'roc_aucs', 'mean_roc_auc']
        ...
        save_checkpoint(val_loss, model, optimizer, epoch, False)

        # Save checkpoint stats in json format for ease of viewing
        save_checkpoint_stats_json({}, epoch, index=False)
