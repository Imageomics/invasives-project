...
if os.path.exists(ARGS.output_path):
    print(f"Output path: {ARGS.output_path} exists. Quitting execution")
    sys.exit()
else:
...
if DATASET == 'invasive-512':
    ## TODO: Using ImageNet mean and std, replace with mean and std of invasive plants training file 
    mean = torch.tensor([0.485, 0.456, 0.406])
    std = torch.tensor([0.229, 0.224, 0.225])
    transform = get_transform(target_size, mean, std, 'squarepad_no_augment_normalize')
    ...
    if ARGS.server == 'pda':
        ...
        raise NotImplementedError('Need to setup for OSC')
    else:
...
# This is for debugging with a subset of the data 
if ARGS.trial:
...
def evaluate(net, dataloader, epoch, type):
    ...
    msg = f"{type} | Epoch: {epoch}/{EPOCH} | Loss: {total_loss / (total + eps)}"
    #| AP: {average_precisions} | MAP: {mean_ap} | ROC_AUC: {mean_roc_auc} | f1: {f1s} | Precs: {precisions} | Recs: {recalls}"
